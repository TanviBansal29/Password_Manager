...
class UserBusiness:
    ...
    def view_all_data(self):
        with DatabaseConnection(Config.DATABASE_NAME) as connection:
            cursor = connection.cursor()
            data = cursor.execute(Config.QUERY_TO_VIEW_ALL_DATA, (self.user_id, )).fetchall()
            if not data:
    ...
    def view_data_by_website(self, website):
        with DatabaseConnection(Config.DATABASE_NAME) as connection:
            cursor = connection.cursor()
            data = cursor.execute(Config.QUERY_TO_VIEW_DATA_BY_WEBSITE, (self.user_id, website, )).fetchall()
            if not data:
                raise DataNotFoundError("No data found")
            return data
